...
void help(char *argv[]) {
	cout
		...
		<< "\n"
		<< "\nDefault Call (with parameters:list = ../stereoData_19-03_list.txt  board_w = 9, board_h = 6, root_path= )"
		<< "\n" << argv[0] << "\n"
		<< "\nManual call:"
		<< "\n" << argv[0] << " <path/list_of_stereo_pairs>]  [<board_w> <board_h> [root_path]"
		<< "\n\n PRESS ANY KEY TO STEP THROUGH RESULTS AT EACH STAGE."
		<< "\n" << endl;
}

static void StereoCalib(const char *imageList, int nx, int ny,bool useUncalibrated, const char* rootpath) {
	...
	for (;;) {
		...
		std::string filepath = std::string(rootpath) + buf;
		cv::Mat img = cv::imread(filepath, 0);
		if (img.empty())
			break;
		imageSize = img.size();
		imageNames[lr].push_back(filepath);
		i++;
